...
def search_papers(
    ...
    跟 discover_papers 不同：
      ...
      - 用 search-mode scoring（少 recency、重 citation/keyword）

    回傳排序後的 top N 個 paper dict。
    """
    ...
    # arXiv 用 all-fields query
    arxiv_query = f'all:"{query}"' if not any(op in query for op in (":", " AND ", " OR ")) else query
    ...
    deduped = _dedupe(arxiv_papers + ss_papers)
    ...
    logger.info(f"search '{query}': {len(arxiv_papers)} arxiv + {len(ss_papers)} ss → {len(deduped)} deduped → top {len(result)}")
    return result
